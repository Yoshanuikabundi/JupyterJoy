...
class BashKernel(object):
    ...
    def do_execute(self, code, silent, store_history=True,
        ...
        if exitcode:
            error_content = {'execution_count': self.kernel.execution_count,
                             'ename': '', 'evalue': str(exitcode), 'traceback': []}

            self.kernel.send_response(self.kernel.iopub_socket, 'error', error_content)
...
class StatefulBashMagics(Magics):
    """Bash magics that hold their state between cells

    invoke with %pbash or %%pbash"""
