...
class Chain(object):
    ...
    def set_ss(self, ss, source="self"):
        ...
        logging.debug(' '.join(str(x) for x in [
            'new sstypes for chain with sequence',
            self.seq,
            'with len',
            len(self.sstypes),
            ':',
    ...
    def cg(self, force=False, com=False):
        ...
        fail     = False
        previous = ''
        logging.debug(' '.join(str(len(i)) for i in (self.residues, self.sstypes, self.sequence)))
        if not len(self.residues) == len(self.sstypes) == len(self.sequence):
            raise ValueError("lengths of stuff to zip don't match")
